...
При каждом обновлении приложения — одна команда:

```bash
./scripts/update.sh
```

Скрипт сам делает `git pull`, пересобирает Docker образ и применяет миграции БД. **Данные не затираются** — они хранятся в Docker volume отдельно от контейнеров.
...
Создаёт файл `backups/backup_2024-01-15_14-30-00.sql.gz`. Бэкапы старше 30 дней удаляются автоматически.

### Восстановление из бэкапа

```bash
./scripts/restore.sh ./backups/backup_2024-01-15_14-30-00.sql.gz
```

### Автоматический бэкап каждую ночь (cron)

На сервере выполните `crontab -e` и добавьте строку (замените путь):

```
0 2 * * * cd /path/to/resource-planner && ./scripts/backup.sh >> ./backups/backup.log 2>&1
```

Бэкап будет создаваться каждую ночь в 02:00.
